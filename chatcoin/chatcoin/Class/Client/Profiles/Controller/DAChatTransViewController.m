...
@interface DAChatTransViewController ()
<UITextFieldDelegate>
...
/** 地址 view */
@property (nonatomic, strong) UITextField *addrTextF;
...
@implementation DAChatTransViewController
...
- (void)balanceViewInit
{
    ...
    moneyImageView.image = [UIImage imageNamed:@"聊天币"];
...
- (void)bigViewInit
{
    ...
    CGFloat bigViewH = ViewHeight * 3 + 2;
    ...
    UILabel *accountLabel01 = [[UILabel alloc] initWithFrame:CGRectMake(15, 0, 42, ViewHeight)];
    accountLabel01.text = @"地址";
    accountLabel01.font = [UIFont systemFontOfSize:18];
    
    [_bigView addSubview:accountLabel01];
    
    CGFloat addrTextFX = accountLabel01.right_LL;
    self.addrTextF = [self cusTextFiledWithFrame:CGRectMake(addrTextFX, 0, 230, ViewHeight) placeholder:@"转账地址"];
    self.addrTextF.keyboardType = UIKeyboardTypeNamePhonePad;
    [_bigView addSubview:self.addrTextF];
    
    
    
    _qcodeBtn = [[UIButton alloc] initWithFrame:CGRectMake(SCREEN_WIDTH - 75, 0, 30, ViewHeight)];
    [_qcodeBtn.imageView setContentMode:UIViewContentModeScaleAspectFit];
    
    [_qcodeBtn setImage:[UIImage imageNamed:@"复制"] forState:UIControlStateNormal];
    [_qcodeBtn addTarget:self action:@selector(qcodeClick:) forControlEvents:UIControlEventTouchUpInside];
    
    [_bigView addSubview:_qcodeBtn];
    
    _peopleBtn = [[UIButton alloc] initWithFrame:CGRectMake(SCREEN_WIDTH - 40, 0, 30, ViewHeight)];
    [_peopleBtn.imageView setContentMode:UIViewContentModeScaleAspectFit];

    [_peopleBtn setImage:[UIImage imageNamed:@"复制"] forState:UIControlStateNormal];
    [_peopleBtn addTarget:self action:@selector(peopleClick:) forControlEvents:UIControlEventTouchUpInside];
    
    [_bigView addSubview:_peopleBtn];
    
    
    
    UIView *line01 = [[UIView alloc] initWithFrame:CGRectMake(10, self.addrTextF.bottom_LL, SCREEN_WIDTH - 10, 0.5)];
    line01.backgroundColor = ZKColor_VarGray(224);
    [_bigView addSubview:line01];
    
    
    
    UILabel *accountLabel02 = [[UILabel alloc] initWithFrame:CGRectMake(15, ViewHeight + 1, 42, ViewHeight)];
    accountLabel02.text = @"金额";
    accountLabel02.font = [UIFont systemFontOfSize:18];
    
    [_bigView addSubview:accountLabel02];
    
    self.moneyTextF = [self cusTextFiledWithFrame:CGRectMake(addrTextFX, ViewHeight + 1, 230, ViewHeight) placeholder:@"最多 88.88"];
    ...
    UILabel *accountLabel03 = [[UILabel alloc] initWithFrame:CGRectMake(15,ViewHeight+ 1 + ViewHeight + 1, 42, ViewHeight)];
    accountLabel03.text = @"密码";
    accountLabel03.font = [UIFont systemFontOfSize:18];
    
    [_bigView addSubview:accountLabel03];
    
    self.passTextF = [self cusTextFiledWithFrame:CGRectMake(addrTextFX, ViewHeight + ViewHeight + 2 , 230, ViewHeight) placeholder:@"钱包密码"];
...
- (void)sureBtnInit
{
    ...
    CGFloat sureBtnY = 10 + ViewHeight + 30 + ViewHeight * 3 + 2 + 30;
    ...
    self.sureBtn.backgroundColor = ZKColor_Var(116, 151, 241);
...
- (void)qcodeClick:(UIButton *)btn
{
    ZLog(@"%@", btn);
}


- (void)peopleClick:(UIButton *)btn
{
    ZLog(@"%@", btn);
}
...
//            unichar character = [string characterAtIndex:loopIndex]; //将输入的值转化为ASCII值（即内部索引值），可以参考ASCII表
//            // 48-57;{0,9};65-90;{A..Z};97-122:{a..z}
//            if (character < 46) return NO; // 46 小数点
//            if (character == 47) return NO; // 47 斜线
